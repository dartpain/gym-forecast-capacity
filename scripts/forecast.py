...
engine = sqlalchemy.create_engine(
    'postgresql://%s:%s@%s:%s/%s' % (username, password, host, port, database),
...
for location in locations:
    print('Processing location: %s' % location['name'])
    df = pd.read_sql_query("SELECT * FROM clubs WHERE name = '"+ location['name'] +"'", con=connection)
    m = Prophet(interval_width=0.95, weekly_seasonality=35, daily_seasonality=35)
    m.add_country_holidays(country_name='UK')
    df.rename(columns={'date_created': 'ds'}, inplace=True)
    df.rename(columns={'percentage': 'y'}, inplace=True)
    df = df[df['name'] == location['name']]
    print(df['ds'].max())
    data_count = df['ds'].count()
    m.fit(df)
    future = m.make_future_dataframe(periods=288, freq='5min', include_history=False)
    forecast = m.predict(future)

    connection.execute("CREATE TABLE IF NOT EXISTS predictions (id VARCHAR(255) PRIMARY KEY, name VARCHAR(255), date_predicted TIMESTAMP, percentage INTEGER)")
    print("Deleting old predictions")
    connection.execute("DELETE FROM predictions WHERE name = '"+ location['name'] +"'")

    print("Uploading new predictions")
    df = forecast[['ds', 'yhat']]
    df['name'] = location['name']
    df.rename(columns={'ds': 'date_predicted'}, inplace=True)
    df.rename(columns={'yhat': 'percentage'}, inplace=True)
    df['id'] = location['id'] + '_' + df['date_predicted'].astype(str)
